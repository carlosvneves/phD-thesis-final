...
graph save Graph "Graph_within_var.gph", replace
restore
...
xtdata, be clear
graph twoway scatter avg ano || lfit avg ano
graph save Graph "Graph_between_var.gph", replace
restore

// Agrupados por etapa
//Custo/km no tempo
graph twoway scatter cust_km ano || lfit cust_km ano, by(etp)
graph save Graph "Graph_cust_km_byetp.gph", replace
//Receita/km no tempo
graph twoway scatter rec_km ano || lfit rec_km ano, by(etp)
graph save Graph "Graph_rec_km_byetp.gph", replace
//Avaliação Geral no tempo
graph twoway scatter avg ano || lfit avg ano, by(etp)
graph save Graph "Graph_avg_byetp.gph", replace

// Por Concessionária
//Custo/km no tempo
graph twoway scatter cust_km ano || lfit cust_km ano, by(conc)
graph save Graph "Graph_cust_km_byconc.gph", replace
//Receita/km no tempo
graph twoway scatter rec_km ano || lfit rec_km ano, by(conc)
graph save Graph "Graph_rec_km_byconc.gph", replace
//Avaliação Geral no tempo
graph twoway scatter avg ano || lfit avg ano, by(conc)
graph save Graph "Graph_avg_km_byconc.gph", replace

// gera variáveis dummy para marcar as etapas
gen dEtapa2 = (etp==2)
gen dEtapa3 = (etp==3)

global x cust_km rec_km tar
// modelo POLS com Erros-Padrão Robustos Clusterizados
quietly reg avg $x dEtapa2 dEtapa3, vce (cluster id)
eststo POLS_rob

// modelo com estimador between
quietly xtreg avg $x dEtapa2 dEtapa3, be
eststo BE

// modelo com estimador efeitos fixos
quietly xtreg avg $x, fe
scalar Fstat = e(F)
...
quietly xtreg avg $x, fe vce (cluster id)
eststo FE_rob

// modelo com estimador efeitos aleatórios
quietly xtreg avg $x dEtapa2 dEtapa3, re
...
xtsum id ano avg cust_km rec_km tar npp ext  
